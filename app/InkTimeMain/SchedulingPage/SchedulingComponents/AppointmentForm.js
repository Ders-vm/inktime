...
import { addAppointment } from '../../_services/InkTimeServices';

function AppointmentForm({ userId, setAppointments, setShowForm, appointments }) {
  ...
  const isOverlap = (newDate, newStartTime, newEndTime, existingAppointments) => {
    if (!Array.isArray(existingAppointments)) {
      return false; // Return false if existingAppointments is not an array
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const appointmentId = await addAppointment(userId, formData);
    // Update appointments state
    setAppointments(prevAppointments => [...prevAppointments, { id: appointmentId, ...formData }]);
    // Reset form data
    setFormData({
      date: '',
      startTime: '00:00',
